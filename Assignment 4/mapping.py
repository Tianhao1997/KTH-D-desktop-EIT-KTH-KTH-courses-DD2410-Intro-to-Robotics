...
class Mapping:
    ...
    def update_map(self, grid_map, pose, scan):
        ...
        r_x = pose.pose.position.x - origin.position.x
        r_y = pose.pose.position.y - origin.position.y
        ...
        for i in range(len(scan.ranges)):
            if scan.ranges[i] <= scan.range_min or scan.ranges[i] >= scan.range_max:
                ...
                occup_x = int((scan.ranges[i] * cos(angle) + r_x) / resolution)
                occup_y = int((scan.ranges[i] * sin(angle) + r_y) / resolution)

                x_list.append(occup_x)
                y_list.append(occup_y)
                occups.append((occup_x, occup_y))
                non_occups = self.raytrace(
                    [int(r_x / resolution), int(r_y / resolution)], [occup_x, occup_y])
                for non_occup in non_occups:
                    (free_x, free_y) = non_occup
    ...
    def inflate_map(self, grid_map):
        ...
        for h in range(height):
            for w in range(width):
                if grid_map[w, h] == self.occupied_space:
                    x = w - rr
                    y = h - rr
                    for r_x in range(2 * rr + 1):
                        for r_y in range(2 * rr + 1):
                            dis = sqrt((x + r_x - w) ** 2 + (y + r_y - h) ** 2)
                            if dis <= rr and grid_map[int(x + r_x), int(y + r_y)] != self.occupied_space and self.is_in_bounds(grid_map, int(x + r_x), int(y + r_y)):
                                self.add_to_map(grid_map, int(
                                    x + r_x), int(y + r_y), self.c_space)
